"""Build an offline skincare creative-test demo for adclip.

The fixture creates two exact Meta-feed creative artifacts, synthetic deployment
records, normalized observations, and one explicit CTR experiment. It performs
no network requests and needs no API keys.
"""
...
def _svg(headline: str, subhead: str, accent: str) -> str:
  ...
  <rect width=\"1080\" height=\"1350\" fill=\"#F6F0E8\"/>
  <rect x=\"72\" y=\"92\" width=\"936\" height=\"1166\" rx=\"36\" fill=\"#FFFFFF\"/>
  <rect x=\"72\" y=\"92\" width=\"18\" height=\"1166\" fill=\"{accent}\"/>
  <circle cx=\"820\" cy=\"360\" r=\"150\" fill=\"#E8DDD1\"/>
  <rect x=\"755\" y=\"265\" width=\"130\" height=\"235\" rx=\"22\" fill=\"#F7F7F4\" stroke=\"#3F5147\" stroke-width=\"5\"/>
  <text x=\"140\" y=\"360\" font-family=\"Arial, sans-serif\" font-size=\"58\" font-weight=\"700\" fill=\"#29352F\">{headline}</text>
  <text x=\"140\" y=\"500\" font-family=\"Arial, sans-serif\" font-size=\"32\" fill=\"#3F5147\">{subhead}</text>
  <text x=\"140\" y=\"1080\" font-family=\"Arial, sans-serif\" font-size=\"30\" font-weight=\"700\" fill=\"#29352F\">20% off first order</text>
  <text x=\"140\" y=\"1140\" font-family=\"Arial, sans-serif\" font-size=\"24\" fill=\"#69756E\">Synthetic adclip creative-test fixture</text>
</svg>
"""
...
def _deployment(campaign_id: str, entry: dict, ad_id: str) -> DeploymentRecord:
    ...
    return DeploymentRecord(
        ...
        external_campaign_id="demo_skincare_launch",
        external_adset_id="demo_skincare_prospecting",
        external_creative_id=f"meta_{ad_id}",
        external_name=f"Synthetic skincare {entry['variant_id']}",
        status="ACTIVE",
        metadata={
            "synthetic": True,
...
def build(output_dir: Path) -> dict[str, object]:
    brief = AdBrief(
        product="Morrow Daily Barrier Moisturizer",
        value_prop=(
            "A lightweight, fragrance-free daily moisturizer with ceramides "
            "and squalane for dry or sensitive skin."
        ),
        audience="Adults 25-40 shopping for a simple daily moisturizer.",
        angles=["plain benefit hook", "vivid problem-framing hook"],
        tone="calm, modern, specific, low-hype",
        cta="Shop 20% off",
        formats=["meta_feed_4x5"],
        variants=2,
        pool_size=2,
        must_include=["20% off first order"],
        must_avoid=["clinically proven", "guaranteed results", "miracle"],
    ...
    control_path = _write_creative(
        ...
        "v01",
        _svg(
            "Daily moisture, kept simple",
            "Ceramides + squalane in a lightweight, fragrance-free formula.",
            "#87998F",
        ),
    )
    treatment_path = _write_creative(
        ...
        "v02",
        _svg(
            "Your moisturizer should not quit before lunch",
            "Lightweight daily moisture for skin that feels dry too soon.",
            "#5F7669",
        ),
    )
    ...
    created = ExperimentApplication().create(
        str(root),
        name="Problem-framing hook CTR test",
        hypothesis=(
            "A vivid problem-framing hook increases click-through rate versus "
            "a plain product-benefit hook."
        ),
        changed_factor="opening hook",
        control_variant_id="v01",
        treatment_variant_id="v02",
        control_value="plain product benefit",
        treatment_value="vivid problem framing",
        primary_metric="ctr",
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        default="./adclip_creative_test_demo",
        help="Demo campaign directory (default: ./adclip_creative_test_demo)",
    )
    ...
    print(
        "adclip performance compare {dir} --since {since} --until {until} "
        "--action-report-time conversion --metric ctr".format(
            dir=result["campaign_dir"],
            since=START.isoformat(),
            until=END.isoformat(),
        )
